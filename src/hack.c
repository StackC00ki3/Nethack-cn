    ...
    boolean costly)
{
...

    {
        ...
        if (!u.usteed) {
            ...
            if (givemesg)
                pline("%s移动%s.", YMonnam(u.usteed), what);
        }
...
cannot_push(struct obj *otmp, coordxy sx, coordxy sy)
{
    if (throws_rocks(gy.youmonst.data)) {
        ...
        if (u.usteed && P_SKILL(P_RIDING) < P_BASIC) {
            You("骑术不够熟练, 无法从%s身边%s%s.", /*修改语序:You("aren't skilled enough to %s %s from %s.",*/
                y_monnam(u.usteed),
                willpickup ? "捡起" : "拨开",
                the(xname(otmp)));
        } else {
...
moverock_core(coordxy sx, coordxy sy)
{
    ...
    while ((otmp = sobj_at(BOULDER, sx, sy)) != 0) {
        ...
        if (Levitation || Is_airlevel(&u.uz)) {
            ...
            if (Blind)
                feel_location(sx, sy);
            You("没有足够的着力点来推动%s.", the(xname(otmp)));
        ...
        if (isok(rx, ry) && !IS_OBSTRUCTED(levl[rx][ry].typ)
            ...
            if (mtmp && !noncorporeal(mtmp->data)
                ...
                if (flags.verbose) {
                    ...
                    if (deliver_part1)
                        pline("也许这就是%s不能移动它的原因.",
                              you_or_steed);
                    else
                        pline("%s不能移动%s.",
                              upstart(you_or_steed), the(xname(otmp)));
                }
                return cannot_push(otmp, sx, sy);
            }
...
still_chewing(coordxy x, coordxy y)
{
    ...
    if (!u.uconduct.food++)
        livelog_printf(LL_CONDUCT,
                       ...
                       boulder ? "一块巨石"
                       : IS_TREE(lev->typ) ? "一棵树"
                         : IS_OBSTRUCTED(lev->typ) ? "岩石"
                           : (lev->typ == IRONBARS) ? "铁栏杆"
                             : "一扇门");
    u.uhunger += rnd(20);
...
dosinkfall(void)
{
    static const char fell_on_sink[] = "坠落到水槽上";
    struct obj *obj;
    int dmg;
                ...
                if (is_db_wall(x, y)) {
                    ...
                    if (sym == S_stone)
                        ...
                        Sprintf(buf, "不可能的 [背景字形=%d]",
                                glyph);
                    pline_dir(xytodir(dx, dy), "那是%s.", buf);
                }
            }
            return FALSE;
        }
 ...
 testdiag:
    ...
    if (sobj_at(BOULDER, x, y) && (Sokoban || !Passes_walls)) {
        if (mode != TEST_TRAV && svc.context.run >= 2
            ...
            if (mode == DO_MOVE && flags.mention_walls)
                pline_dir(xytodir(dx,dy), "一块巨石挡住了你的路.");
            return FALSE;
        }
        if (mode == DO_MOVE) {
    ...
    struct trap *desttrap) /* nonnull if another trap at <x,y> */
{
    ...
    case TT_BEARTRAP:
        if (flags.verbose) {
            predicament = "夹在捕熊陷阱中";
            if (u.usteed)
                Norep("%s被%s.", upstart(steedname), predicament);
            else
 ...
 wriggle_free:
            if (u.usteed)
                pline("%s终于%s.", upstart(steedname),
                      !anchored ? "挣脱出来" : "把铁球拽了出来");
            else
                You("终于%s.",
                    !anchored ? "挣脱出来" : "把铁球拽了出来");
            if (anchored)
...
domove_fight_web(coordxy x, coordxy y)
{
    ...
    if (svc.context.forcefight && trap && trap->ttyp == WEB && trap->tseen) {
        ...
        if (uwep && (u_wield_art(ART_STING)
            ...
            /* guaranteed success */
            pline("%s%s了蛛网!", bare_artifactname(uwep),
    ...
    coordxy x, coordxy y)
{
    ...
    if (mtmp->mtrapped && is_pit(trap->ttyp)
        ...
        You("停下.  %s无法脱离%s.", YMonnam(mtmp), what);
        ...
        case Trap_Killed_Mon:
             ...
             */
            {
                ...
                if (!u.uconduct.killer++)
                    livelog_printf(LL_CONDUCT, "第一次杀死生物");
...
{
    static const char unknown_obstacle[] = "一个未知障碍物";
...
avoid_trap_andor_region(coordxy x, coordxy y)
{
    ...
    if (ParanoidTrap && !Blind && !Stunned && !Confusion && !Hallucination
        ...
        Snprintf(qbuf, sizeof qbuf, "进入那团%s吗?",
    ...
    if (ParanoidTrap && !Stunned && !Confusion
        ...
        Snprintf(qbuf, sizeof qbuf, "真的要%s那个%s吗?",
                 into ? "进入" : "踏上",
...
invocation_message(void)
{
    ...
    if (invocation_pos(u.ux, u.uy) && !On_stairs(u.ux, u.uy)) {
        ...
        if (u.usteed)
            Sprintf(buf, "在%s下方", y_monnam(u.usteed));
