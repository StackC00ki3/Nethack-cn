...
staticfn const char *
getpos_display_goal(const char *goal)
{
    if (!goal || !strcmp(goal, "desired location"))
        return "期望的位置";
    if (!strcmp(goal, "a monster"))
        return "怪物";
    if (goal == what_is_a_location)
        return "怪物, 物品或位置";
    return goal;
}
    ...
    int gloc)
{
    ...
    if (iflags.getloc_usemenu)
        Sprintf(sbuf, "使用'%s'/'%s'获取%s%s的菜单.",
                k1, k2, filtertxt, gloc_descr[gloc][3]);
    else
        Sprintf(sbuf, "使用'%s'/'%s'%s%s%s.",
                k1, k2, move_cursor_to, filtertxt, gloc_descr[gloc][2]);
...
getpos_help(boolean force, const char *goal)
{
    ...
    boolean goal_is_monster = (goal && !strcmp(goal, "a monster"));
    const char *display_goal = getpos_display_goal(goal);
    ...
    Sprintf(sbuf,
            ...
            visctrl(cmd_from_func(do_move_east)), display_goal);
    putstr(tmpwin, 0, sbuf);
    Sprintf(sbuf,
            "使用'%s', '%s', '%s', '%s'快速移动光标, %s.",
 ...
 skip_non_mons:
    ...
    if (!force)
        putstr(tmpwin, 0, "完成后输入空格或Escape.");
    putstr(tmpwin, 0, "");
...
getpos_menu(coord *ccp, int gloc)
{
    ...
    Sprintf(tmpbuf, "选择一个%s%s%s",
            gloc_descr[gloc][1],
            gloc_filtertxt[iflags.getloc_filter],
            iflags.getloc_travelmode ? "作为旅行目的地" : "");
...
getpos(coord *ccp, boolean force, const char *goal)
{
    ...
    display_goal = getpos_display_goal(goal);
    if (flags.verbose) {
        pline("(如需说明请输入'%s')",
 ...
 do_rushrun:
        ...
        if (c == gc.Cmd.spkeys[NHKF_GETPOS_HELP] || redraw_cmd(c)) {
            ...
            pline("%s自动描述已%s.",
                  flags.verbose ? "光标下事物的" : "",
                  iflags.autodescribe ? "开启" : "关闭");
            if (!iflags.autodescribe)
                show_goal_msg = TRUE;
            msg_given = TRUE;
            goto nxtc;
