...
precheck(struct monst *mon, struct obj *obj)
{
    ...
    if (obj->oclass == POTION_CLASS) {
        coord cc;
        static const char *const empty = "那瓶药水原来是空的.";
        struct monst *mtmp;
    ...
    if (obj->oclass == WAND_CLASS && obj->cursed
        ...
        if (vis) {
            pline_mon(mon, "%s挥动%s, 它突然爆炸了!", Monnam(mon),
                  an(xname(obj)));
        } else {
    ...
    boolean self)
{
    ...
    if (!canseemon(mtmp)) {
        ...
        You_hear("%s传来挥杖声.", (mdistu(mtmp) <= range * range)
                                 ? "附近" : "远处");
        ...
        pline_mon(mtmp, "%s挥动%s!", Monnam(mtmp), an(xname(otmp)));
        stop_occupation();
    }
    otmp->spe -= 1;
}
    ...
    boolean self)
{
    ...
    if (!canseemon(mtmp)) {
        ...
        pline("%s把%s对准自己吹奏!", Monnam(mtmp), an(objnamp));
...
mreadmsg(struct monst *mtmp, struct obj *otmp)
{
    ...
    if (vismon) {
        ...
        pline_mon(mtmp, "%s阅读%s!", Monnam(mtmp), onambuf);
        ...
        Snprintf(blindbuf, sizeof blindbuf, "%s%s",
                 mtmp->mconf ? "试着念诵" : "念诵", onambuf);
        You_hear("%s%s.",
...
mquaffmsg(struct monst *mtmp, struct obj *otmp)
{
    if (canseemon(mtmp)) {
        observe_object(otmp);
        pline_mon(mtmp, "%s喝下%s!", Monnam(mtmp), singular(otmp, doname));
    } else if (!Deaf) {
    ...
    int how)            /* type of that object */
{
    if (tele_restrict(mtmp)) { /* mysterious force... */
        ...
        if (vismon)
            pline_mon(mtmp, "%s一时间显得迷惑.", Monnam(mtmp));
    } else {
...
mon_escape(struct monst *mtmp, boolean vismon)
{
    if (mon_has_special(mtmp)
        ...
        return 0;
    if (vismon)
        pline_mon(mtmp, "%s逃出了地下城!", Monnam(mtmp));
    mongone(mtmp);
    return 2;
}
...
#define m_flee(m)                          \
    ...
    case MUSE_UNICORN_HORN:
        ...
        if (vismon) {
            if (otmp)
                pline_mon(mtmp, "%s使用独角兽角!", Monnam(mtmp));
            else
                pline_The("%s的角尖发光了!", mon_nam(mtmp));
        }
        if (!mtmp->mcansee) {
            ...
            if (vismon)
                pline_mon(mtmp, "%s现在看起来稳当多了.", Monnam(mtmp));
        } else {
    ...
    case MUSE_BUGLE:
        ...
        if (vismon) {
            pline_mon(mtmp, "%s吹奏%s!", Monnam(mtmp), doname(otmp));
        } else if (!Deaf) {
    ...
    case MUSE_SCR_TELEPORTATION: {
        ...
        if (obj_is_cursed || mtmp->mconf) {
            ...
            if (mon_has_amulet(mtmp) || In_endgame(&u.uz)) {
                if (vismon)
                    pline_mon(mtmp, "%s一时间显得非常迷惑.",
                          Monnam(mtmp));
            } else if (nlev == depth(&u.uz)) {
                if (vismon)
                    pline_mon(mtmp, "%s颤抖了一下.", Monnam(mtmp));
            } else {
                get_level(&flev, nlev);
    ...
    case MUSE_WAN_DIGGING:
        ...
        if (!Can_dig_down(&u.uz) && !levl[mtmp->mx][mtmp->my].candig) {
            ...
            if (vis) {
                seetrap(t);
                pline_mon(mtmp, "%s在%s上挖出了一个坑.", Monnam(mtmp),
                      surface(mtmp->mx, mtmp->my));
            }
        ...
        seetrap(t);
        if (vis) {
            pline_mon(mtmp, "%s在%s上挖出了一个洞.", Monnam(mtmp),
                  surface(mtmp->mx, mtmp->my));
            pline_mon(mtmp, "%s%s了下去...", Monnam(mtmp),
                  is_flyer(mtmp->data) ? "飞" : "掉");
        } else if (!Deaf) {
            Soundeffect(se_crash_through_floor, 100);
            You_hear("%s穿过%s的声音.", something,
                     surface(mtmp->mx, mtmp->my));
        }
        fill_pit(mtmp->mx, mtmp->my);
    ...
    case MUSE_TRAPDOOR:
        ...
        if (vis) {
            pline_mon(mtmp, "%s进入了一个%s!", Monnam(mtmp),
    ...
    case MUSE_UPSTAIRS:
        ...
        if (Inhell && mon_has_amulet(mtmp) && !rn2(4)
            ...
            if (vismon)
                pline_mon(mtmp, "%s向楼上逃走了!", Monnam(mtmp));
    ...
    case MUSE_DOWNSTAIRS:
        ...
        if (!stway)
            return 0;
        if (vismon)
            pline_mon(mtmp, "%s向楼下逃走了!", Monnam(mtmp));
    ...
    case MUSE_UP_LADDER:
        ...
        if (!stway)
            return 0;
        if (vismon)
            pline_mon(mtmp, "%s沿梯子向上逃走了!", Monnam(mtmp));
    ...
    case MUSE_DN_LADDER:
        ...
        if (!stway)
            return 0;
        if (vismon)
            pline_mon(mtmp, "%s沿梯子向下逃走了!", Monnam(mtmp));
    ...
    case MUSE_SSTAIRS:
        ...
        if (vismon)
            pline_mon(mtmp, "%s沿%s楼梯逃走了!", Monnam(mtmp),
                  stway->up ? "向上" : "向下");
    ...
    case MUSE_TELEPORT_TRAP:
        ...
        t = t_at(gt.trapx, gt.trapy);
        if (vis) {
            pline_mon(mtmp, "%s移动到一个%s上!", Monnam(mtmp),
                  trapname(t->ttyp, FALSE));
        }
        /* if trap was in a concealed niche, it's no longer concealed */
        reveal_trap(t, vis);
    ...
    case MUSE_POT_FULL_HEALING:
        ...
        if (!mtmp->mcansee && otmp->otyp != POT_SICKNESS)
            mcureblindness(mtmp, vismon);
        if (vismon)
            pline_mon(mtmp, "%s看起来完全康复了.", Monnam(mtmp));
        if (oseen)
            makeknown(otmp->otyp);
        m_useup(mtmp, otmp);
        return 2;
        ...
        if (hits_you) {
            ...
            if (mtmp->ispriest && *in_rooms(mtmp->mx, mtmp->my, TEMPLE)) {
                if (cansee(mtmp->mx, mtmp->my))
                    pline_mon(mtmp, "%s抵抗了魔法!", Monnam(mtmp));
...
use_offensive(struct monst *mtmp)
{
    ...
    case MUSE_SCR_EARTH: {
        ...
        /* Identify the scroll */
        if (canspotmon(mtmp)) {
            /*修改语序: 中文里怪物名需位于“周围/上方”之前*/
            pline_The("%s在%s%s隆隆作响!", ceiling(mtmp->mx, mtmp->my),
                      mon_nam(mtmp), otmp->blessed ? "周围" : "上方");
            if (oseen)
                makeknown(otmp->otyp);
    ...
    case MUSE_SCR_FIRE: {
        ...
        if (mtmp->mconf) {
            ...
            if (vis)
                pline_The("卷轴爆发出一道火柱!");
            shieldeff(mtmp->mx, mtmp->my);
            pline_mon(mtmp, "%s没有受伤.", Monnam(mtmp));
        ...
        if (can_carry(mon, xobj)) {
            if (vismon) {
                if (howfar > 2) /* not adjacent */
                    Norep("%s在%s里翻找.", Monnam(mon), contnr_nam);
                else if (takeout_indx == 0) /* adjacent, first item */
                    /*修改语序: 中文里容器名需先于取出的物品名*/
                    pline_mon(mon, "%s从%s中取出了%s.", Monnam(mon),
                          contnr_nam, doname(xobj));
                else /* adjacent, additional items */
                    pline("%s又取出了%s.", upstart(mpronounbuf),
                          doname(xobj));
            }
...
use_misc(struct monst *mtmp)
{
 ...
 skipmsg:
                if (vismon) {
                    pline_mon(mtmp, "%s看起来不安.", Monnam(mtmp));
                    trycall(otmp);
                }
                m_useup(mtmp, otmp);
                return 2;
        ...
        if (vismon)
            pline_mon(mtmp, "%s看起来更有经验了.", Monnam(mtmp));
        if (oseen)
    ...
    case MUSE_POT_POLYMORPH:
        ...
        m_useup(mtmp, otmp);
        if (vismon)
            pline_mon(mtmp, "%s突然变异了!", Monnam(mtmp));
    ...
    case MUSE_POLY_TRAP:
        ...
        if (vismon || vistrapspot) {
            pline_mon(mtmp, "%s故意移动到%s上!", Some_Monnam(mtmp),
                  t->tseen ? trapname(t->ttyp, FALSE) : "一个隐藏的陷阱");
    ...
    case MUSE_BULLWHIP:
        ...
        {
            const char *The_whip = vismon ? "那条牛鞭" : "一条鞭子";
            int where_to = rn2(4);
            struct obj *obj = uwep;
            const char *hand;
            ...
            case 1: /* onto floor beneath mon */
                pline_mon(mtmp, "%s把%s从你的%s中拽走!", Monnam(mtmp),
                          the_weapon, hand_buf);
                place_object(obj, mtmp->mx, mtmp->my);
                break;
            case 2: /* onto floor beneath you */
                pline_mon(mtmp, "%s把%s拽到%s上!", Monnam(mtmp),
                          the_weapon, surface(u.ux, u.uy));
                dropy(obj);
                break;
            case 3: /* into mon's inventory */
                pline_mon(mtmp, "%s抢走了%s!", Monnam(mtmp), the_weapon);
...
you_aggravate(struct monst *mtmp)
{
    ...
    display_self();
    You_feel("被%s激怒.", noit_mon_nam(mtmp));
    display_nhwindow(WIN_MAP, TRUE);
    docrt();
    if (unconscious()) {
        gm.multi = -1;
        gn.nomovemsg = "你被激怒了, 猛然完全清醒过来.";
    }
    newsym(mtmp->mx, mtmp->my);
    if (!canspotmon(mtmp))
        map_invisible(mtmp->mx, mtmp->my);
    ...
    boolean stoning) /* T: stop petrification, F: cure stun && confusion */
{
    ...
    if (vis) {
        ...
        obj->quan = 1L;
        if (obj->oclass == POTION_CLASS)
            pline_mon(mon, "%s喝下%s.", Monnam(mon),
                  distant_name(obj, doname));
        else if (obj->otyp == TIN)
            pline_mon(mon, "%s打开%s并吃掉里面的东西.", Monnam(mon),
                  distant_name(obj, doname));
        else
            pline_mon(mon, "%s吃掉%s.", Monnam(mon),
                  distant_name(obj, doname));
        obj->quan = save_quan;
    } else if (!Deaf)
        You_hear("%s.",
    ...
    if (acid && !tinned && !resists_acid(mon)) {
        mon->mhp -= rnd(15);
        if (vis)
            pline_mon(mon, "%s胃酸严重发作.", Monnam(mon));
        if (DEADMONSTER(mon)) {
...
{                   /* [by_you not honored if 'mon' triggers fire trap]. */
    ...
    if (trap) {
        ...
        if (mon->mx == trap->tx && mon->my == trap->ty) {
            ...
            newsym(mon->mx, mon->my);
            if (vis)
                pline("%s%s%s火焰陷阱!", Mnam,
                      is_floater(mon->data) ? "飘过" : "踏上",
                      trap->tseen ? "那个" : "一个");
        }
        ...
        if (vis)
            pline_mon(mon, "%s.",
                      monverbself(mon, Monnam(mon), "喷吐", "火焰烧向自己"));
        if (!rn2(3))
        ...
        if (vis) {
            ...
            pline("%s喝下了一瓶燃烧的%s.",
                  saw_lit ? upstart(strcpy(Pronoun, "它")) : Monnam(mon),
                  simpleonames(obj));
            makeknown(POT_OIL);
        }
    ...
    if (dmg) {
        ...
        if (DEADMONSTER(mon)) {
            if (by_you) {
                ...
                if (vis)
                    pline_mon(mon, "%s被火焰%s了!", Monnam(mon),
            ...
            if (vis)
                pline_mon(mon, "%s被烧伤了%s", Monnam(mon), exclam(dmg));
        }
    }
    if (vis) {
        if (res && !DEADMONSTER(mon))
            pline_mon(mon, "%s身上的黏液被烧掉了!", Monnam(mon));
