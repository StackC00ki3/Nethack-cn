...
throw_obj(struct obj *obj, int shotlimit)
{
    ...
    if (is_art(obj, ART_MJOLLNIR) && obj != uwep) {
        pline("%s必须先拿在手里才能投掷.", The(xname(obj)));
        res = ECMD_OK;
        goto unsplit_stack;
    }
    ...
    if (gm.m_shot.i < gm.m_shot.n) {
        if (verbose && !svc.context.mon_moving) {
            You("在第%d次%s之后停止%s.",
                gm.m_shot.i,
                gm.m_shot.s ? "射击" : "投掷",
                gm.m_shot.s ? "射击" : "投掷"); /*修改语序:You("停止%s在%d%s%s之后.", gm.m_shot.s ? "射击" : "投掷", gm.m_shot.i, ordin(gm.m_shot.i), gm.m_shot.s ? "射击" : "投掷");*/
        }
    ...
    boolean verbosely) /* usually True; False if caller has given drop mesg */
{
    ...
    if (IS_ALTAR(levl[u.ux][u.uy].typ)) {
        ...
        const char *verb = (obj->otyp == WAN_STRIKING) ? "击中" : "碰到";
...
hurtle_step(genericptr_t arg, coordxy x, coordxy y)
{
    ...
    if (!Passes_walls || !(may_pass = may_passwall(x, y))) {
        ...
        if (IS_OBSTRUCTED(levl[x][y].typ)
            ...
            why = IS_TREE(ltyp) ? "撞到树"
                  : IS_OBSTRUCTED(ltyp) ? "撞到墙"
                    : odoor_diag ? "撞到门框"
                      : "撞到关着的门";
            if (odoor_diag)
                You("撞到了门框!");
    ...
    if (is_pool(x, y) && !u.uinwater) {
        if (is_waterwall(x, y) || !(Levitation || Flying || Wwalking)) {
            ...
            Norep("你越过了%s.", is_moat(x, y) ? "护城河" : "水池");
        }
        ...
        if (touch_petrifies(mon->data) && !(uarmu || uarm || uarmc)) {
            Snprintf(svk.killer.name, sizeof svk.killer.name,
                     ...
                     x_monnam(mon, mon->mtame ? ARTICLE_YOUR : ARTICLE_A,
                              "飞撞的", EXACT_NAME | SUPPRESS_NAME, FALSE));
            instapetrify(svk.killer.name);
            newsym(u.ux, u.uy);
        }
    }
...
toss_up(struct obj *obj, boolean hitsroof)
{
    ...
    if (!has_ceiling(&u.uz)) {
        action = "飞入"; /* 飞入“天空”或“上方的水中” */
    } else if (hitsroof) {
        if (breaktest(obj)) {
            pline("%s打中了%s.", Doname2(obj), ceiling(u.ux, u.uy));
            breakmsg(obj, !Blind);
    ...
    if (obj->oclass == POTION_CLASS) {
        ...
        case EGG:
            if (petrifier && !Stone_resistance
                ...
                if (uarmh)
                    Your("%s没能保护你.", helm_simple_name(uarmh));
                goto petrify;
            }
            FALLTHROUGH;
            /*FALLTHRU*/
 ...
 petrify:
        ...
        if (is_silver && Hate_silver)
            pline_The("银器灼烧了你!");
        if (harmless)
            hit(thesimpleoname(obj), &gy.youmonst, ", 但没有造成伤害.");

        hitfloor(obj, TRUE);
        gt.thrownobj = 0;
        if (!harmless)
            losehp(dmg, "掉落的物体", KILLED_BY_AN);
    }
    return TRUE;
}
    ...
    struct obj *oldslot) /* for thrown-and-return used with !fixinv */
{
    ...
    if (!gt.thrownobj) {
        ...
        if (flooreffects(obj, gb.bhitpos.x, gb.bhitpos.y, "落下")) {
            throwit_return(TRUE);
            return;
        }
        ...
        if (mon && mon->isshk && is_pick(obj)) {
            if (cansee(gb.bhitpos.x, gb.bhitpos.y))
                pline("%s抓起%s.", Monnam(mon), the(xname(obj)));
    ...
    struct obj *obj) /* gt.thrownobj or gk.kickedobj or uwep */
{
    ...
    if (obj->oclass == GEM_CLASS && is_unicorn(mon->data)
        ...
        if (helpless(mon)) {
            ...
            pline("%s接住并扔掉了%s.", Monnam(mon), the(xname(obj)));
            return 0;
        } else {
    ...
    if (obj->oclass == WEAPON_CLASS || is_weptool(obj)
        ...
        Strcpy(trail,
               digests(md) ? "的肠子" : is_whirly(md) ? "的气流" : "");
        monname = mon_nam(mon);
        if (*trail)
...
breakmsg(struct obj *obj, boolean in_view)
{
    ...
    case POT_WATER: /* really, all potions */
        if (!in_view)
            You_hear("%s破碎了!", something);
        else
            pline("%s破碎%s%s!", Doname2(obj),
...
throw_gold(struct obj *obj)
{
    ...
    if (u.uswallow) {
        ...
        if (digests(u.ustuck->data))
            ...
            swallower = strcat(s_suffix(swallower), "的内脏");
