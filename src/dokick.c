...
really_kick_object(coordxy x, coordxy y)
{
    ...
    if (slide && !Blind)
        pline("啊！ %s 从%s上滑过。", Doname2(gk.kickedobj),
...
otransit_msg(struct obj *otmp, boolean nodrop, boolean chainthere, long num)
{
    ...
    if (num || chainthere) {
        ...
        if (num) { /* means: other objects are impacted */
            Sprintf(xbuf, " %s %s东西%s", otense(otmp, "击中"),
                    (num == 1L) ? "另一个" : "其他", (num > 1L) ? "" : "");
        } else { /* chain-only msg */
            Sprintf(xbuf, " %s 了你的锁链", otense(otmp, "哗啦作响"));
        }
        if (nodrop)
            Sprintf(eos(xbuf), "死亡");
        else
            Sprintf(eos(xbuf), "然后%s了%s。",
