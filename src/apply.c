...
use_leash_core(struct obj *obj, struct monst *mtmp, coord *cc, int spotmon)
{
    if (!spotmon && !glyph_is_invisible(levl[cc->x][cc->y].glyph)) {
        ...
        if (mtmp->mleashed) {
            pline("这%s%s已经被拴着了.", mon_classifier(mtmp),
...
use_candelabrum(struct obj *obj)
{
    ...
    if (obj->spe <= 0) {
        ...
        pline("这%s%s没有%s.", classifier(obj), xname(obj), s);
...
use_lamp(struct obj *obj)
{
    ...
    if ((!Is_candle(obj) && obj->age == 0)
        ...
        if (obj->otyp == BRASS_LANTERN) {
            ...
            pline("这%s%s燃尽了.", classifier(obj), xname(obj));
        }
        return;
    }
    if (obj->cursed && !rn2(2)) {
...
fig_transform(anything *arg, long timeout)
{
    ...
    if (mtmp) {
        ...
        Sprintf(monnambuf, "一%s%s", mon_classifier(mtmp), m_monnam(mtmp));
        and_vanish[0] = '\0';
        if ((mtmp->minvis && !See_invisible)
...
use_whip(struct obj *obj)
{
 ...
 whipattack:
        ...
        if (otmp) {
            ...
            Sprintf(onambuf, "%s%s%s", (otmp->quan == 1L) ? "一" : "", (otmp->quan == 1L) ? classifier(otmp) : "", cxname(otmp)); //Strcpy(onambuf, cxname(otmp));
            if (gotit) {
...
use_royal_jelly(struct obj **optr)
{
    ...
    if (obj->cursed) {
        if (eobj->timed || eobj->corpsenm != oldcorpsenm)
            pline("这%s%s在微弱地%s.", classifier(obj), xname(eobj), otense(eobj, "颤抖"));
        else
            pline("%s", nothing_seems_to_happen);
        kill_egg(eobj);
        goto useup_jelly;
